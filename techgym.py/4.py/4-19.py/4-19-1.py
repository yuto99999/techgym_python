...
class Human(Player):
  ...
  def bet(self):
    bet_message1 = '何枚BETしますか？：(1-99)'
    bet_coin = input(bet_message1)
    while not self.enable_bet_coin(bet_coin):
      bet_coin = input(bet_message1)
    bet_message2 = 'どこにBETしますか？：(R,B,1-8)'
    bet_str = input(bet_message2)
    while not self.enable_bet_cell(bet_str):
      bet_str = input(bet_message2)
