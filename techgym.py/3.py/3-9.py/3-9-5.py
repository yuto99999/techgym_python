...
teams = []
playing_team = {'myself':False, 'enemy':False}
...
def choice_team(player):
    ...
    playing_team[player] = teams[choice_team_number - 1]
    print(player_name + 'のチームは「' + playing_team[player].name + '」です')

def get_play_inning(inning):
    if inning == 'front':
        hit_rate = playing_team['myself'].get_hit_rate()
        out_rate = playing_team['enemy'].get_out_rate()
    elif inning == 'back':
        hit_rate = playing_team['enemy'].get_hit_rate()
        out_rate = playing_team['myself'].get_out_rate()
...
def play():
    ...
    for i in range(9):
        ...
        playing_team['myself'].total_score += inning_score

        if i == 8 and playing_team['myself'].total_score < playing_team['enemy'].total_score:
            score_boards[2] += 'X｜'
        else:
            inning_score = get_play_inning('back')
            score_boards[2] += str(inning_score) + '｜'
            playing_team['enemy'].total_score += inning_score
    score_boards[0] += 'R｜'
    score_boards[1] += str(playing_team['myself'].total_score) + '｜'
    score_boards[2] += str(playing_team['enemy'].total_score) + '｜'
